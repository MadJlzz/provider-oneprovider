...
import (
	...
	apisCluster "github.com/MadJlzz/provider-oneprovider/apis/cluster"
	apisNamespaced "github.com/MadJlzz/provider-oneprovider/apis/namespaced"
	"github.com/MadJlzz/provider-oneprovider/config"
	"github.com/MadJlzz/provider-oneprovider/internal/clients"
	controllerCluster "github.com/MadJlzz/provider-oneprovider/internal/controller/cluster"
	controllerNamespaced "github.com/MadJlzz/provider-oneprovider/internal/controller/namespaced"
	"github.com/MadJlzz/provider-oneprovider/internal/features"
	"github.com/MadJlzz/provider-oneprovider/internal/version"
)
...
func main() {
	var (
		app                     = kingpin.New(filepath.Base(os.Args[0]), "Terraform based Crossplane provider for OneProvider").DefaultEnvars()
	...
	log := logging.NewLogrLogger(zl.WithName("provider-oneprovider"))
	if *debug {
	...
	mgr, err := ctrl.NewManager(cfg, ctrl.Options{
		...
		LeaderElectionID: "crossplane-leader-election-provider-oneprovider",
		Cache: cache.Options{
	...
	kingpin.FatalIfError(apisCluster.AddToScheme(mgr.GetScheme()), "Cannot add cluster-scoped OneProvider APIs to scheme")
	kingpin.FatalIfError(apisNamespaced.AddToScheme(mgr.GetScheme()), "Cannot add namespaced OneProvider APIs to scheme")
	...
	if canSafeStart {
		...
		kingpin.FatalIfError(controllerCluster.SetupGated(mgr, clusterOpts), "Cannot setup cluster-scoped OneProvider controllers")
		kingpin.FatalIfError(controllerNamespaced.SetupGated(mgr, namespacedOpts), "Cannot setup namespaced OneProvider controllers")
	} else {
		log.Info("Provider has missing RBAC permissions for watching CRDs, controller SafeStart capability will be disabled")
		kingpin.FatalIfError(controllerCluster.Setup(mgr, clusterOpts), "Cannot setup cluster-scoped OneProvider controllers")
		kingpin.FatalIfError(controllerNamespaced.Setup(mgr, namespacedOpts), "Cannot setup namespaced OneProvider controllers")
	}
